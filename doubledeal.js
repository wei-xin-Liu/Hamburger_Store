...
for (let index = 1; index < 16; index++) {
    dob_img[dob_img.length] = document.getElementById(`dob_img${index}`).src;
    dob_pay[dob_pay.length] = parseInt(document.getElementById(`dob_pay${index}`).innerText.substring(1));
}
localStorage.setItem("dob_img", dob_img);
localStorage.setItem("dob_pay", dob_pay);
...
function buy(i) {
    if (document.getElementById("mySidebar1").getElementsByTagName("img").length > 0) {
        ...
        if ( total_img.indexOf(dob_img[i - 1]) >= 0) {
            total_img = total_img;
            total_pay = total_pay;
            total_btt = total_btt;
            count = count - 1;
            ...
            total_img[total_img.length] = dob_img[i - 1];
            total_pay[total_pay.length] = dob_pay[i - 1];
