...
for (let index = 1; index < 18; index++) {
    sig_img[sig_img.length] = document.getElementById(`sig_img${index}`).src;
    sig_pay[sig_pay.length] = parseInt(document.getElementById(`sig_pay${index}`).innerText.substring(1));
}
localStorage.setItem("sig_img", sig_img);
localStorage.setItem("sig_pay", sig_pay);
...
function buy(i) {
    if (document.getElementById("mySidebar1").getElementsByTagName("img").length > 0) {
        ...
        if ( total_img.indexOf(sig_img[i - 1]) >= 0) {
            total_img = total_img;
            total_pay = total_pay;
            total_btt = total_btt;
            count = count - 1;
            ...
            total_img[total_img.length] = sig_img[i - 1];
            total_pay[total_pay.length] = sig_pay[i - 1];
            total_btt[total_btt.length] = 1;
        }
