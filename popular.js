...
for (let index = 1; index < 19; index++) {
    pop_img[pop_img.length] = document.getElementById(`pop_img${index}`).src;
    pop_pay[pop_pay.length] = parseInt(document.getElementById(`pop_pay${index}`).innerText.substring(1));
}
localStorage.setItem("pop_img", pop_img);
localStorage.setItem("pop_pay", pop_pay);
...
function buy(i) {
    if (document.getElementById("mySidebar1").getElementsByTagName("img").length > 0) {
        ...
        if (total_img.indexOf(pop_img[i - 1]) >= 0) {
            total_img = total_img;
            total_pay = total_pay;
            total_btt = total_btt;
            count = count - 1;
            ...
            total_img[total_img.length] = pop_img[i - 1];
            total_pay[total_pay.length] = pop_pay[i - 1];
